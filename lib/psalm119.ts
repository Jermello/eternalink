/**
 * Psalm 119 (Tehillim 119) — Hebrew name → liturgical reading.
 *
 * Psalm 119 is an alphabetic acrostic: 22 sections, one per Hebrew letter, each
 * with 8 verses whose words begin with that letter. The traditional "name
 * memorial" custom is to read the sections matching each letter of the
 * deceased's Hebrew name, in order.
 *
 * The text below is the complete Masoretic Tanakh (public domain), with full
 * niqqud and te'amim (cantillation marks). Letter-extraction strips all marks
 * in the U+0591–U+05C7 range before matching, so naming the sections still
 * works regardless of how niqqud is supplied in the Hebrew name input.
 *
 * Exports:
 *   - HEBREW_LETTERS: the 22 letters in alphabetic order.
 *   - PSALM_119: each letter → its section (name + 8 verses + opening verse #).
 *   - extractHebrewLetters(name): pulls Hebrew letters from a string in order,
 *     stripping niqqud / cantillation, normalising final-form letters
 *     (ך→כ, ם→מ, ן→נ, ף→פ, ץ→צ).
 *   - getPsalmReading(name): the ordered list of sections to display.
 */
...
export type PsalmSection = {
  letter: HebrewLetter;
  letterName: string; // transliterated name, e.g. "Aleph"
  verseStart: number; // 1-indexed verse number this section opens at
  verses: string[];
};

export const PSALM_119: Record<HebrewLetter, PsalmSection> = {
  "א": {
    letter: "א",
    letterName: "Aleph",
    verseStart: 1,
...
// Final-form Hebrew letters → their base form, so they map to a section.
const FINAL_FORMS: Record<string, HebrewLetter> = {
  "ך": "כ",
  "ם": "מ",
  "ן": "נ",
  "ף": "פ",
...
const HEBREW_LETTER_SET = new Set<string>(HEBREW_LETTERS);

/**
 * Extract the ordered Hebrew letters from a name. Strips niqqud (vowel marks)
 * and te'amim (cantillation), normalises final-form letters, and ignores any
 * non-Hebrew character. Letters appear in the order they occur in the name —
 * including duplicates, since the traditional reading repeats sections when
 * a letter repeats.
 */
...
/**
 * Build the Psalm-119 reading for a Hebrew name. Returns one PsalmSection per
 * letter, in name order, including duplicates.
 */
